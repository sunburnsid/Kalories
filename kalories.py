...
def getSuggestions():
    ...
    print "decoded"
<<<<<<< HEAD
    clarifai_api = ClarifaiApi() # assumes environment variables are set. APP_SECRET='qIf-5HaEzO225zuUaj5FTREU7iYAJLHU5_XrpHH_'
    result = clarifai_api.tag_images(open('static/cheese.jpg', 'rb'))
=======
    ...
    result = clarifai_api.tag_images(open('/static/cheese.jpeg', 'rb'))
>>>>>>> 806a6f0ad5c633851358adb26f6199d48cde7a06
    print "api call"
    ...
    for val in parsed:
        print val
        if (db.session.query(Food).filter(Food.name.contains(val)).scalar() is not None):
            answer.append(val)
